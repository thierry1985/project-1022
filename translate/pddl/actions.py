...
class DurativeAction(object):
    ...
    def parse(alist):
        ...
        if len(duration_list) == 1:
            #R.Huang  this special case is hacked
            item = duration_list[0]
            assert item[0] in ("<=",">=","=")
            assert len(item) == 3
            assert item[1] == "?duration"
            if item[2].__class__.__name__ == "list":
                simple_duration = conditions.Fluent( conditions.parse_literal(item[2]), -1 ) 
            else:
                simple_duration = item[2]
        #The original handling logics
        ...
        for rest in iterator:
            assert False, rest
        action =  DurativeAction(name, parameters, (duration_start,duration_end), condition, effect)
        action.simple_duration = simple_duration
        return action
    parse = staticmethod(parse)
    ...
    def instantiate(self, var_mapping, init_facts, fluent_facts, init_function_vals,
        ...
        #R.Huang
        simple_duration = None
        str_dur = str( self.simple_duration )
        str_dur = "".join(str_dur.split("."))
        if str_dur.isdigit():
          simple_duration = self.simple_duration
        else:
          in_dur = self.simple_duration.instantiate( self.simple_duration.atom, var_mapping)
          
          for func in task.initial_fluents:
            if func.atom.predicate == in_dur.atom.predicate:
              cpp = tuple([str(item) for item in in_dur.atom.args])
              if func.atom.args == cpp:
                str_dur = func.value
            
          str_dur = "".join(str_dur.split("."))
          if not str_dur.isdigit():
            print str_dur, "NOT FOUND!"
            assert False, "Unkown Error."
          simple_duration = str_dur
        #End of R.Huang
        ...
        if effects:
            action = PropositionalDurativeAction(name, inst_duration, inst_conditions, effects)
            assert simple_duration != None
            action.simple_duration = simple_duration
            return action
        else:
            return None
