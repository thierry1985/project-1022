...
class Task(object):
  ...
  def parse(domain_pddl, task_pddl):
    ...
    task_name, task_domain_name, objects, init, init_fluents, goal = parse_task(task_pddl)
    ...
    task.initial_fluents = init_fluents
    return task
...
class DerivedFunctionAdministrator(object):
    ...
    def get_derived_function(self,exp):
        ...
        def get_new_symbol(key,*lengths):   
        ...
        if isinstance(exp,f_expression.PrimitiveNumericExpression):
            ...
            args = subexp.args
            if key not in self.functions:
                symbol = get_new_symbol(key,len(subexp.args))
            ...
            if key not in self.functions:
                if exp.op in ("+","*"):
                    ...
                    args = pne1.args + pne2.args
                if key not in self.functions:
                    symbol = get_new_symbol(key,len(pne1.args),len(pne2.args))
...
def parse_task(task_pddl):
  ...
  for fact in init[1:]:
    if fact[0] == "=":
        fluent = conditions.Fluent( conditions.Atom(fact[1][0], fact[1][1:]), fact[2])
        initial_fluents.append( fluent )        
