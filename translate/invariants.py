...
class Invariant:
  ...
  def check_action_balance(self, balance_checker, action, enqueue_func):
    ...
    if isinstance(action,pddl.Action):
      all_del_effects = [[eff for eff in action.effects if isinstance(eff.peffect,pddl.NegatedAtom)]]
      all_add_effects = [[eff for eff in action.effects if isinstance(eff.peffect,pddl.Atom)]]
    else: #if isinstance(action,pddl.DurativeAction)
      all_del_effects = [[eff for eff in action.effects[0] if isinstance(eff.peffect,pddl.NegatedAtom)],
                         [eff for eff in action.effects[1] if isinstance(eff.peffect,pddl.NegatedAtom)]]
      all_add_effects = [[eff for eff in action.effects[0] if isinstance(eff.peffect,pddl.Atom)],
                         [eff for eff in action.effects[1] if isinstance(eff.peffect,pddl.Atom)]]
    for del_effects,add_effects in zip(all_del_effects,all_add_effects):
      matched_add_effects = []
      for eff in add_effects:
        ...
        if part:
          for previous_part, previous_literal in matched_add_effects:
            if previous_part.matches(part, previous_literal, eff.peffect) \
               and previous_literal != eff.peffect: # "Airport" has duplicate effects
              return False
